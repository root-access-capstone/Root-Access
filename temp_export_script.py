...
"""
    ...
    with db.engine.connect().execution_options(autocommit=True) as conn:
        try:
            ...
            print(df.tail())
            df.to_csv('./root_access_db_export-3-23-22.csv')
        except Exception as error: # ONE error handler for storing
            # Don't want to store partial data, could cause duplicates
            print('bad things: ', error)

if __name__=="__main__":
    query_table('sensor_data')
